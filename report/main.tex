...
\end{align}
Using the computed loss, we can optimize the parameters $\theta$ in order to minimize the loss.
...
What is more, because of bootstrapping and function approximation the algorithm can forget some useful behaviour learned in the past.
This can lead to overfitting, instability in the training process, and consequently poor generalization.
What is more, it may also fail to cover a significant amount of state space, and may get stuck
in the local optima because of this.
To break the correlation, we can use a replay buffer, which is a buffer that stores experiences.
Using this, we no longer have to learn from subsequent experiences, we can get random samples from the replay buffer
and learn from them.
Breaking the correlation between subsequent samples helps us to improve coverage and generalization of the algorithm~\cite{DBLP:books/sp/Plaat22}.
...
In order to improve the stability of learning and reduce the divergence cause by frequent weight updates, we can introduce
a target network~\cite{DBLP:books/sp/Plaat22}.
Target network $\hat Q$ is a new neural network that the algorithm will use to output the target values.
The newly added network is structurally a copy of the value network $Q$, that gets updated every $n$ steps
by copying the parameters of the neural network $Q$ that we use for action selection ($\hat Q = Q$).
That way the target predictions remain stable for a longer period of time.
To calculate the target value, we can still use the \autoref{eq:calculating-target}, but instead of using the $Q_\theta$
neural network we need to replace it with $Q_{\theta - i}$ neural network where $\theta - i$ stands for the parameters
