...
Using the computed loss, the parameters can be optimized $\theta$ in order to minimize the loss.
...
and whose parameters can be tuned to incorporate a balance between exploration and exploitation.
...
The loss is equal to the mean squared difference between the target $Q$ value and the $Q(s,a)$ value that was computed.
...
Using this, the DQN agent no longer has to learn from subsequent experiences, as it can obtain random samples
...
After every $n$ steps, the $\hat Q_{\theta - i}$ is updated, making the variable $i$ equal to 0 and $\hat Q_{\theta}$
equal to $Q_\theta$.
