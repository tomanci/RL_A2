...
% TODO mention ER == Experience Replay
...
% TODO: mention TN == Target Network
...
In the \autoref{dqn-ablation-study}, the effect of experience replay and a target network are investigated.
...
we decided to run each combination of hyper-parameters five times as a compromise between statistical significance and the computational power required to perform the experiments. The same number of repetitions is done in every plot of this section if not mentioned otherwise.\\
...
\end{figure}
   ...
   Five different architectures have been tested as the \autoref{comp_nn_arch} reports. 
...
In this subscetion we ablation study is performed \autoref{fig_ablation}. After having found the best configuration for the hyper-parameters, we are comparing the different solutions to improve the DQN algorithm.\\
...
\label{sec:bonus}
So far, only some individual hyperparameters were analyzed in isolation in \autoref{results}.
...
support most of the findings from \autoref{results}. 
